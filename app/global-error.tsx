...
export default function GlobalError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <html lang="fr">
      <body style={{ background: '#15100c', color: '#f7f1e4', fontFamily: 'Arial, sans-serif' }}>
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, textAlign: 'center', padding: 24 }}>
          ...
          <button
            onClick={reset}
            style={{ border: 0, borderRadius: 999, padding: '12px 22px', background: '#ef4c19', color: 'white', fontWeight: 800, cursor: 'pointer' }}
          >
            Réessayer
          </button>
        </div>
